 ...
 * By David Blanco
 *
 * Contact: codecanyon.net/user/davidbo90
 *
 * Created: June 6, 2013
 *
 * Copyright (c) 2013, db. All rights reserved.
 * Released under CodeCanyon License codecanyon.net/
 *
 * Note: Script based in jQuery Masonry v2.1.07 made by David DeSandro masonry.desandro.com/ (under MIT)
 *
 * ======================================================= */
  ...
  $.fn.grid = function( options ) {
    ...
    var db = function($this){
        ...
        var fixLoadMoreButton = function(result){
            ...
            if(result){
                loadMore.addClass('grid-loadMore').html('LOAD MORE STREAMS');
            }else{
                loadMore.removeClass('grid-loadMore').html('');
            }
        };

        var makeBox = function(category, imgName, thumb, imgData){
            ...
            var img_url = stream_url + '/' + imgName;
            //console.log(img_url);
            var box ='<div class="gbox" data-url="'+imgData.url+'" data-category="'+category+'">'+
                          '<img src="'+img_url+'" data-lightbox="'+$directory+'/'+categoryURL+imgName+'" />'+
        ...
        var loadImages = function(numberImagesToLoad){
                ...
                for(var i=0; i<max; i++){
                    ...
                    for(var key in rsJSON){
                      ...
                      for (var img in category) {
                          ...
                          if(cont2 == i){
                            //CUSTOMIZATION-->
                            collection.push({ 'category':keyTmp, 'title':category[img].title, 'url':category[img].url, 'image':img, 'thumb': category[img].thumb, 'order': category[img].order, 'categoryOriginal': key  });
                            break;
                          }
                          cont2++;
                      }
            ...
            while(num<numberImagesToLoad){
                ...
                boxes += makeBox(obj['category'], obj['image'], obj['thumb'], obj );
        ...
        $.getJSON(base_url+"/streamreader?directory="+$directory+'&categoriesOrder='+ops.categoriesOrder+'&imagesOrder='+ops.imagesOrder,function(data){ 
            rsJSON = data;
